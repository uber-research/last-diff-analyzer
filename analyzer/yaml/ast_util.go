...
// astForest stores yaml ASTs for a given change set in a form
...
// astForestBuild builds a forest of ASTs for all specified yaml
// files.
func (a *Analyzer) astForestBuild(filesToAnalyze []string, rootDir string) (astForest, error) {
	...
	for _, f := range filesToAnalyze {
		if a.IsAnalyzable(f) {
			...
			if err != nil {
				return forest, fmt.Errorf("failed to create a yaml AST for %q: %v", srcFilePath, err)
			}
...
// astBuild construct and returns an AST for a yaml file at a given path.
func astBuild(filePath string) (*yaml.Node, error) {
	var ast yaml.Node
	dat, err := ioutil.ReadFile(filePath)
	if err != nil {
		return nil, err
	}
	err = yaml.Unmarshal(dat, &ast)
	if err != nil {
		return nil, err
	}
...
// astForestEq determines equivalence of the yaml AST forests.
func (a *Analyzer) astForestEq() (bool, error) {
	if len(a.baseForest.fileNames) != len(a.lastForest.fileNames) ||
		...
		return false, errors.New("lists of yaml changes to compare have different length")
	}

	for i, baseAst := range a.baseForest.asts {
		if a.baseForest.fileNames[i] != a.lastForest.fileNames[i] {
			return false, errors.New("lists of yaml files to compare are different")
		}
