...
// astForest stores go.mod file ASTs and file names for a given
...
// astForestBuild builds a forest of ASTs for all go.mod files.
func (a *Analyzer) astForestBuild(filesToAnalyze []string, rootDir string) (astForest, error) {
	...
	for _, f := range filesToAnalyze {
		if a.IsAnalyzable(f) {
			...
			if err != nil {
				return forest, fmt.Errorf("failed to create a go.mod AST for %q: %v", srcFilePath, err)
			}
...
// astBuild construct and returns an AST for a go.mod file at a given path.
func astBuild(filePath string) (*modfile.File, error) {
	dat, err := ioutil.ReadFile(filePath)
	if err != nil {
		return nil, err
	}
	ast, err := modfile.Parse(filePath, dat, nil)
	if err != nil {
		return nil, err
	}
...
// astForestEq determines equivalence of the go.mod AST forests.
func (a *Analyzer) astForestEq() (bool, error) {
	if len(a.baseForest.fileNames) != len(a.lastForest.fileNames) ||
		...
		// we build both yaml AST forests based on the same (single)
		// list of files, so this should naver happen
		return false, errors.New("lists of go.mod file changes to compare have different length")
	}

	for i, baseAst := range a.baseForest.asts {
		if a.baseForest.fileNames[i] != a.lastForest.fileNames[i] {
			return false, errors.New("lists of go.mod files to compare are different")
		}
